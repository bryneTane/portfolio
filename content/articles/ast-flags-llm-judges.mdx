...
[Lumorem](https://lumorem.com) audits a repo and returns a short, ranked list of performance findings. Two stages:
...
`eslint-plugin-react-perf` fires on every inline allocation in a JSX prop. Right call for a yellow squiggle in your editor.

First version: install the plugin, hand every finding to the LLM. Audit on [MirrAI](https://mirraitryon.com) (~100 components): **228 candidates, 224 rejected, 4 confirmed**. 97% false positives. Every rejected candidate still costs an LLM call, and a false positive that _survives_ the LLM costs some investigation time to the user.
...
```ts
// eslint-bridge/gates.ts (excerpt)
...
Rule the gate follows: **drop with confidence, keep when in doubt**. Same [MirrAI](https://mirraitryon.com) repo:
...
```ts
// technologies/react/detectors/context-overconsumption/detector.ts
...
```ts
// shared/project-index.ts (excerpt)
...
More cross-file detectors are queued (graphql overfetch, n+1 query, ...). Each one extends the same project-index shape.
...
```ts
// shared/llm-review.ts (excerpt)

const CHUNK_SIZE = 10;
// For a file with N candidates: ceil(N/10) calls, file content cached.
```

The system prompt is per-detector. `react-perf` candidates need "is this inline allocation actually expensive?"; `react_context_overconsumption` candidates need "is splitting this context the right fix?" Same output schema, different rails.
...
```ts
// technologies/react/manifest.ts
